...
class montecarlo:
    ...
    def minimum(self):
        ...
        self._stock_unit_price=pd.DataFrame(self.simulation())
        
        return {"Minimum Price":self._stock_unit_price.iloc[-1].min(),"Minimum Return %": 100*(self._stock_unit_price.iloc[-1].min()-self._stock_unit_price.iloc[0,0])/self._stock_unit_price.iloc[0,0]}      
    
    def maximum(self):
        
        self._stock_unit_price=pd.DataFrame(self.simulation())
        
        return  {"Maximum Price":self._stock_unit_price.iloc[-1].max(),"Maximum Return %": 100*(self._stock_unit_price.iloc[-1].max()-self._stock_unit_price.iloc[0,0])/self._stock_unit_price.iloc[0,0]}
...
class stock:
    def __init__(self,ticker,start_date,end_date):
        ...
        self._raw_data=yf.download(self.ticker,self.start_date,self.end_date)['Adj Close']
